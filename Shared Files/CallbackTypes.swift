...
*/
@objc public class UberError : NSError, CustomDebugStringConvertible, JSONCreateable
{
	...
	init(code: String, message: String, fields: [NSObject: AnyObject]?, response: NSURLResponse?)
	{
		self.errorCode = code
	...
	public convenience init?(JSON: [NSObject : AnyObject], response: NSURLResponse?)
	{
		...
		{
			self.init(code: code, message: JSON["message"] as? String ?? "", fields: JSON["fields"] as? [NSObject : AnyObject], response: response)
			return
		}
		return nil
	}
	public convenience required init?(JSON: [NSObject : AnyObject])
	{
		if let code = JSON["code"] as? String
		{
			self.init(code: code, message: JSON["message"] as? String ?? "", fields: JSON["fields"] as? [NSObject : AnyObject], response: nil)
			return
		}
		return nil
	}
