...
internal class UberUserAuthenticator : NSObject
{
	...
	private func requestAccessToken(allowsRefresh: Bool = true) -> String?
	{
		...
		if expiration! > NSDate(timeIntervalSinceNow: 0)
		{
			return accessToken
		}
	...
	private func performFetchForAuthData(request: NSURLRequest)
	{
		...
		let task = session.dataTaskWithRequest(request, completionHandler: {(data, response, error) in
			...
			{
				self.errorHandler?(UberError(JSONData: data, response: response) ??  UberError(error: error, response: response) ?? unknownError)
			}
		})
		task.resume()
	}
	private func parseAuthDataReceived(authData: NSData)
	{
		...
		{
			errorHandler?(UberError(JSONData: authData) ?? unknownError)
		}
	}
