...
@objc public final class UberRequestReceipt : NSObject, JSONCreateable
{
	...
	@objc public override var description : String { return "Receipt for trip \(timeDuration) and \(distance)\(distanceUnits) long. Total Cost: \(totalCharged)\(currencyCode)" }
...
@objc public final class UberReceiptDetail : NSObject, JSONCreateable
{
	...
	@objc public override var description : String { return "Receipt Detail \(name) amounted to \(amount)" }
	
	public init?(JSON: [NSObject : AnyObject])
	{
		guard let name = JSON["name"] as? String, let amount = JSON["amount"] as? Double, let rawType = JSON["type"] as? String
		else
		{
