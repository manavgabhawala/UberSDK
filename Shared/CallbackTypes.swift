...
public protocol UberObjectHasImage
{
...
internal let unknownError = UberError(code: "Unknown", message: "Unknown", fields: nil, response: nil, errorResponse: nil, JSON: [NSObject: AnyObject]())
...
@objc public class UberError : NSError, JSONCreateable
{
	...
	class func createErrorsWithJSON(JSON: [NSObject: AnyObject], response: NSURLResponse?) -> [UberError]
	{
		guard let errors = JSON["errors"] as? [[NSObject : AnyObject]]
		else
		{
			return []
		}
		var parsedErrors = [UberError]()
		for error in errors
		{
			parsedErrors.append(UberError(code: error["code"] as? String ?? "", message: error["message"] as? String ?? error["title"] as? String ?? "", fields: error["fields"] as? [NSObject : AnyObject], response: response, errorResponse: error["status"] as? Int, JSON: JSON))
		}
		return parsedErrors
	}
}
