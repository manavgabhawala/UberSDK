...
extension UberObjectHasImage
{
	...
	public func downloadImage(completionBlock success: (NSImage, Self) -> Void, errorHandler failure : UberErrorHandler?)
	{
		...
		if let data = NSData(contentsOfURL: documentsURL.URLByAppendingPathComponent(URL.lastPathComponent!)), let image = NSImage(data: data)
		{
			success(image, self)
			return
		}
		...
		let task = session.downloadTaskWithRequest(NSURLRequest(URL: URL)) { downloadedURL, response, error in
			...
			do
			{
				...
				guard let image = NSImage(contentsOfURL: fileURL)
